    ...
    return (context) => {
        return new Promise((resolve, reject) => {
            ...
            context.collection.insertMany(documents, null, (err, r) => {
                ...
                context.log(context.createPass(`Inserted ${chalk.bold(r.insertedCount)} documents`));
                resolve(context);
            });
        });
    };
            ...
            context.db.collection(versionCollectionName,
                ...
                (err, collection) => {
                    ...
                    collection.insertOne(newVersion, null, (err, r) => {
                        ...
                        context.log(context.createPass(`Wrote ${chalk.bold.cyan(JSON.stringify(newVersion))} into ${chalk.bold.underline(versionCollectionName)}`));
                        resolve(context);
                    });
                });
        });
...
let checkVersion = function(version){
    return (context) => {
        return new Promise((resolve, reject) => {
            let stepName = `Require version ${chalk.bold(version)}`;
            context.db.collection(versionCollectionName,
                {strict : false},
                (err, collection) => {
                    if(err){
    ...
    return (context) => {
        return new Promise((resolve, reject) => {
            context.collection.deleteMany({}, null, (err, r) => {
                ...
                context.log(context.createPass(`Removed ${chalk.bold(r.deletedCount)} documents`));
                resolve(context);
            });
        });
    };
