...
let dropCollection = function(collectionName){
    return (context) => {
        return new Promise((resolve, reject) => {
            Promise.resolve()
            .then(() => {
            ...
            .then(() => {
                context.db.dropCollection(collectionName)
                .then(result => {
                    context.log(context.createPass(`Collection ${chalk.bold.underline(collectionName)} removed`));
                    if(context.collectionName === collectionName){
                        context.collectionName = undefined;
                        context.removeCollection();
                    }
                    resolve(context);
                })
                .catch(err => {
            ...
            .catch(err => {
                context.log(context.createFail(`Dropping Collection ${chalk.bold.underline(collectionName)}`));
                reject({
                    error : err,
                    context : context
                })
...
let repair = function(){
    return (context) => {
        return new Promise((resolve, reject) => {
            Promise.resolve()
            .then(() => {
                context.db.command({repairDatabase:1})
                .then(result => {
                    context.log(context.createPass(`Database repaired`));
                    resolve(context);
                })
                .catch(err => {
                    throw err;
            ...
            .catch(err => {
                context.log(context.createFail(`Repair database`));
                reject({
                    error : err,
                    context : context
                })
